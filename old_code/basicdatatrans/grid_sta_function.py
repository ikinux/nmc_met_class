...
import nmc_met_class.basicdatatrans as bf
import pandas as pd
...
def transform(grd):
    ...
    sta = pd.DataFrame(dat,index=ids,columns=['lon','lat','data'])
    return sta
...
def linearInterpolation(grd,sta):
    ...
    c10 = (1-dx) * dy
    c11 = dx * dy
    sta1['data']= c00 * grd.dat[jg,ig] + c01 * grd.dat[jg,ig+1] + c10 * grd.dat[jg+1,ig] + c11 * grd.dat[jg+1,ig+1]
    return sta1
...
def cubicInterpolation(grd,sta):
    ...
    for p in range(-1,3,1):
        ...
        for q in range(-1,3,1):
            ...
            fdy = cubic_f(q,dy)
            fdxy = fdx * fdy
            sta1['data'] +=  fdxy * grd.dat[jjq,iip]
    return sta1
